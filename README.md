...
| `npm run version:bump`  | Zvýší denní release verzi (`YY.MM.DD.N`) a synchronizuje API version soubor |
...
- Hodnota se bere z `version.json` ve formátu `YY.MM.DD.N`.
- Hodnota se synchronizuje do `api/src/version.ts` skriptem `npm run version:sync`.
- CI při merge do `main` automaticky zvýší `N` pro aktuální den (`version:bump`) před deployem.
